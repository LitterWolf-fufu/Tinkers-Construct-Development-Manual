...
`ingredient` 消耗的物品或物品类命名空间。键名`item`或`tag`。
...
`leftover` 当一个物品提供的（锭）的值超过制造部件所需的值时，溢出的物品或物品类命名空间。键名`item`或`tag`
...
`fluid` 浇筑需要的流体内容。`amount`所需的流体量。`name`或`tag`所需的流体的命名空间
...
`result` 输出的流体内容。`amount`输出的流体量，`fluid`输出的流体命名空间
